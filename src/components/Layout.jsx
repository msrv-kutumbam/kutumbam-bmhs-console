...
const Layout = ({ content, settings, userData, setUserData, showHeadder }) => {
  ...
  const Sidebar = () => (
    ...
    `}>
      <div 
      className={`h-16  flex items-center justify-between  px-4 md:w-full`}>
      ...
      <div className=" p-4 hidden md:block">
        <button 
          ...
          } logout-trigger ${settings.theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}
          onClick={() => setIsLogoutModalOpen(true)}
        >
          <span
            className={`w-[35px] h-[35px] rounded-md flex items-center justify-center ${
              userData?.profileImageUrl ? '' : 'bg-sky-400'
            }`}
          >
  ...
  const LogoutModal = ({ setIsLogoutModalOpen, handleLogout }) => {
    const isDarkTheme = settings.theme === 'dark';
    return (
      <div
        className={`fixed inset-0 ${
          isDarkTheme ? 'bg-black bg-opacity-80' : 'bg-white bg-opacity-80'
        } flex items-center justify-center z-50`}
      >
        <div
          id="logout-modal"
          className={`rounded-lg shadow-xl w-96 overflow-hidden ${
        ...
        >
          ...
          >
            <h2 className={`text-xl font-semibold ${isDarkTheme ? 'text-red-400' : 'text-red-700'}`}>
              Confirm Logout
            </h2>
          </div>
  ...
  return (
    <div className={`flex flex-col md:flex-row h-screen ${settings.theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-white text-gray-800'}`}>
        ...
        { showHeadder &&  <header className={`h-16 gap-1 flex items-center justify-between px-4 py-3 ${settings.theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center w-full">
            ...
            <input
              ...
              type="search"
              placeholder="Search..." 
              className={`${settings.theme === 'dark' ? 'bg-gray-700 text-gray-300' : 'bg-gray-50 text-gray-800'} w-full py-2 border rounded-lg focus:outline-none focus:border-blue-900`}
          ...
          <div className="flex items-center gap-4">
            <button 
              className="p-2"
             style={{ width: userData?.profileImageUrl ? "56px" : "36px", height: userData?.profileImageUrl ? "56px" : "36px", borderRadius: "0.5em", display: "flex", justifyContent: "center", textAlign: "center", verticalAlign: "middle"}}
            ...
            > 
              {profileImage ? (
                ...
                <div className={`rounded-full flex items-center justify-center text-2xl cursor-pointer`} >
                    {avatar}
                </div>
              )}
            </button>
      ...
      {isLogoutModalOpen && <LogoutModal setIsLogoutModalOpen={setIsLogoutModalOpen} handleLogout={handleLogout}  />}

      {isProfileModalOpen && <Profil 
                                setAvatar={setAvatar} 
                                avatar={avatar} 
                                handleLogout={handleLogout} 
                                userInfo={userData} 
                                modal={setIsProfileModalOpen} 
                                settings={settings} 
