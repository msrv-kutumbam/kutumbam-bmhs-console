...
import { X, Printer, Download, AlertTriangle, Ship, Train, Clock, Share2 } from 'lucide-react';
...
const DetailsShowPopUP = ({ data, isLoading = false, error = null, onClose, collectionType }) => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.save(`${reportType}-report-${new Date().toISOString()}.pdf`);
    } catch (err) {
  ...
  const handleShare = async () => {
    try {
      const shareData = {
        title: `${reportType === 'rake' ? 'Rake Loading' : reportType === 'vessel' ? 'Vessel Operation' : 'Details'} Report`,
        text: `Report for ${data.rakeNo || data.Vessel_name || 'item'}`,
            ...
            <div key={index} className="border-l-4 border-yellow-500 pl-3 py-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-gray-800">
                  {delay.from ? new Date(delay.from).toLocaleString() : 'N/A'} - 
                  {delay.to ? new Date(delay.to).toLocaleString() : 'N/A'}
                </span>
              </div>
              <div className="flex justify-between">
              ...
              <div>
                ...
                {reportType === 'vessel' && (
                  <>
                    ...
                    <p>Berthing Time: {formatVesselDateTime(data.berthing_time)}</p>
                  </>
                )}
              </div>
              <div>
